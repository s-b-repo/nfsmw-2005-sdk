...
/* Validate that the loaded module is speed.exe (image base 0x00400000). */
static inline int nfsmw_validate_speed_exe(void) {
    ...
    return image_base == NFSMW_IMAGE_BASE;
}

/* Write to a code/data page with VirtualProtect-temporary-RWX wrap. */
static inline void nfsmw_write_bytes(uintptr_t addr, const void *src, size_t n) {
    DWORD old;
...
static inline void  nfsmw_write_byte (uintptr_t addr, uint8_t v)  { nfsmw_write_bytes(addr, &v, 1); }
static inline void  nfsmw_write_word (uintptr_t addr, uint16_t v) { nfsmw_write_bytes(addr, &v, 2); }
static inline void  nfsmw_write_int  (uintptr_t addr, uint32_t v) { nfsmw_write_bytes(addr, &v, 4); }
static inline void  nfsmw_write_float(uintptr_t addr, float v)    { nfsmw_write_bytes(addr, &v, 4); }
